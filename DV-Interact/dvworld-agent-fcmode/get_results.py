...
# 只导出图片类文件
VISUAL_SUFFIXES = {".png", ".jpg", ".jpeg", ".gif", ".bmp", ".svg", ".pdf"}
...
def _collect_visual_artifacts(instance_dir: Path, data: dict) -> set[Path]:
    """Collect paths to artifacts (images/csv/xlsx) that should be copied."""
    visual_paths: set[Path] = set()

    for file_path in instance_dir.rglob("*"):
        if not file_path.is_file():
            continue
        if file_path.suffix.lower() in VISUAL_SUFFIXES:
...
def _maybe_copy_user_sim(instance_dir: Path, target_dir: Path) -> None:
    """
    Copy user_simulation.json if present under dvworld/.
    """
    src = instance_dir / "dvworld" / "user_simulation.json"
    if not src.exists():
        # legacy layout: directly under instance_dir
        src = instance_dir / "user_simulation.json"
        if not src.exists():
            return
    dst = target_dir / "user_simulation.json"
...
def process_instance(instance_dir: Path, target_dir: Path, traj_mode: str = TRAJ_MODE_DEFAULT) -> Optional[str]:
    """Process a single interact instance directory."""
    # 优先使用 dvworld 结构；兼容老的 spider 结构
    result_file = instance_dir / "dvworld" / "result.json"
    if not result_file.exists():
        legacy = instance_dir / "spider" / "result.json"
        if legacy.exists():
            result_file = legacy
        else:
            return f"Skipping {instance_dir.name}: missing dvworld/spider result.json"
    ...
    _maybe_copy_user_sim(instance_dir, target_dir)
...
def generate_visualization_results(
    ...
    if traj_mode in (TRAJ_MODE_FILTER, TRAJ_MODE_FILTER_ALIAS):
        target_folder = output_root / f"{folder_name}_f"
    else:
    ...
    for item in sorted(input_folder.iterdir()):
        ...
        error_message = process_instance(item, target_instance_dir, traj_mode=traj_mode)
        if error_message:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "folder_prefix",
        help="Prefix of the output folder(s), matches all dirs starting with it (e.g., Doubao-Seed-1.6-thinking).",
    )
    parser.add_argument(
        "--output_dir",
        type=Path,
        ...
        help=(
            ...
            "Defaults to evaluation_suite/results next to repo root."
        ),
    )
    parser.add_argument(
        "--mode",
...
def main() -> None:
    ...
    default_output_dir = (
        ...
        if args.output_dir
        else script_dir.parent / "evaluation_suite" / "results"
    )
