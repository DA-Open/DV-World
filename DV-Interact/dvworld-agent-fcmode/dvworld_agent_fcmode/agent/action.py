...
class AskUser(Action):
    action_type: str = field(default="ask_user", init=False, repr=False)
    message: str = field(metadata={"help": "question to ask the user"})

    @classmethod
    def get_action_description(cls) -> str:
        return """
## AskUser
* Signature: AskUser(message="your question")
* Description: Ask the user a clarification question and receive a natural language reply.
"""

    @classmethod
    def parse_action_from_text(cls, text: str) -> Optional[Action]:
        match = re.search(r'AskUser\(\s*message\s*=\s*["\'](.*?)["\']\s*\)', text, flags=re.DOTALL | re.IGNORECASE)
        if match:
            return cls(message=remove_quote(match.group(1)))
        match = re.search(r'AskUser\(\s*["\'](.*?)["\']\s*\)', text, flags=re.DOTALL | re.IGNORECASE)
        if match:
            return cls(message=remove_quote(match.group(1)))
        return None

    def __repr__(self) -> str:
        preview = self.message.strip()
        if len(preview) > 2000:
            preview = preview[:2000] + "...[truncated]"
        return f"{self.__class__.__name__}(message={json.dumps(preview, ensure_ascii=False)})"
