...
def _prepare_payload_model_tweaks(model: str, payload: dict) -> dict:
    tweaked = payload
    if model in ["gpt-5-2025-08-07", "o3-2025-04-16", "gpt-5-mini-2025-08-07"] and "temperature" in payload:
    ...
    if model in qwen_no_thinking or model in qwen_limit_max_tokens:
        ...
        if model in qwen_no_thinking:
            # Qwen OpenAI-compatible endpoints require enable_thinking=false (do not drop).
...
def _http_chat_completion(
    ...
    for _ in range(3000):
        ...
        if response.status_code == HTTPStatus.OK and choices:
            ...
            # OpenAI-compatible endpoints may return invalid_request_error and explicitly say enable_thinking is unrecognized.
            if (
                "enable_thinking" in error_message
                and isinstance(tweaked_payload, dict)
                and "enable_thinking" in tweaked_payload
                and not removed_enable_thinking
            ):
                # For Qwen endpoints, force-disable instead of removing.
                tweaked_payload = tweaked_payload.copy()
                tweaked_payload["enable_thinking"] = False
                removed_enable_thinking = True
                logger.warning("Set enable_thinking=False and retrying due to server rejection.")
                time.sleep(0.2)
                continue
...
def _bytedance_chat_completion(payload: dict, model_settings: dict) -> Tuple[bool, dict]:
    ...
    qwen_coder_models = {"openai_qwen3-coder-plus"}
    include_tools = bool(payload.get("tools")) and model_name not in qwen_coder_models
...
def call_llm(payload):
    ...
    if model == "openai_qwen3-coder-plus":
        payload = payload.copy()
