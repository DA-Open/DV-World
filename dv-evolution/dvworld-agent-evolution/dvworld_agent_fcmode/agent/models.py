...
def _get_ipv4_as_14_digits() -> str:
    try:
...
def _new_logid() -> str:
    ...
    ip_part = _get_ipv4_as_14_digits()
    rand_part = f"{random.randint(0, 0xFFFFFF):06X}"
    return f"{ts}{ip_part}{rand_part}"
...
def _http_chat_completion(
    ...
    for _ in range(3000):
        ...
        headers_local["X-TT-LOGID"] = logid
        _write_llm_debug(tweaked_payload, api_url)
        try:
            response = requests.post(
                api_url,
                headers=headers_local,
                json=tweaked_payload,
                proxies=proxies,
            )
        except requests.RequestException as exc:
            logger.error("Failed to call LLM (logid=%s): %s", logid, exc)
        ...
        try:
            ...
            logger.error(
                "LLM response is not valid JSON (logid=%s): %s",
                logid,
                response.text[:200],
            )
        ...
        if response.status_code == HTTPStatus.OK and choices:
            ...
            if code_value in {"InvalidParameter", "invalid_parameter", "-4003"} and isinstance(tweaked_payload, dict):
                if error_message:
                    logger.warning("HTTP InvalidParameter (logid=%s): %s", logid, error_info)
                ...
                if not removed_enable_thinking and "enable_thinking" in tweaked_payload:
                    tweaked_payload = tweaked_payload.copy()
                    tweaked_payload.pop("enable_thinking", None)
                    removed_enable_thinking = True
                    time.sleep(0.2)
                    continue
            ...
            if code_value == "content_filter":
                ...
                return False, {"error": code_value, "logid": logid}
                ...
                logger.error(
                    "Unexpected LLM response (logid=%s, status %s): %s",
                    logid,
...
def _bytedance_chat_completion(payload: dict, model_settings: dict) -> Tuple[bool, dict]:
    ...
    for _ in range(3000):
        ...
        try:
            response = requests.post(api_url, headers=headers_local, json=request)
        except requests.RequestException as exc:
            logger.error("Bytedance HTTP call failed (logid=%s): %s", logid, exc)
        ...
        try:
            ...
            logger.error(
                "Bytedance response is not valid JSON (logid=%s): %s",
                logid,
                response.text[:200],
            )
        ...
        if response.status_code == HTTPStatus.OK and choices:
            ...
            logger.error("Bytedance API error (logid=%s): %s", logid, error_info)
            if code_value == "content_filter":
                _append_content_filter_note(payload)
            elif code_value == "context_length_exceeded":
                return False, {"error": code_value, "logid": logid}
                ...
                invalid_param_retries += 1
                if error_message:
                    logger.warning("Bytedance InvalidParameter (logid=%s): %s", logid, error_info)
                if "enable thinking" in error_message and "stream" in error_message:
