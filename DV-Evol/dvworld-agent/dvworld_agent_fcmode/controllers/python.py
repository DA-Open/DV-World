...
from dvworld_agent_fcmode import renderer
...
"""
    ...
    def render_visualization(self, file_path: str, tool_type: str, output_path: str = "result.png") -> str:
        if not file_path:
            return "render_chart: file_path is required."
        code_path = self.get_real_file_path(file_path)
        if not code_path.exists():
            return f"render_chart: file not found: {file_path}"
        normalized_type = getattr(renderer, "_normalize_type", lambda t: (t or "").lower())(tool_type)
        if not normalized_type:
            normalized_type = renderer.load_case_type(code_path.parent)
        if not normalized_type:
            return "render_chart: tool_type is required (echarts/vega-lite/d3.js/plotly.js)."
        if normalized_type == "python":
            return "render_chart: skip for python; save figures directly in your Python code."

        out_path = self.get_real_file_path(output_path or "result.png")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        try:
            result = renderer.render_and_extract(
                tool_type=normalized_type,
                code_path=code_path,
                out_png=out_path,
            )
        except Exception as exc:  # noqa: BLE001
            return f"render_chart failed: {exc}"

        try:
            rel_out = os.path.relpath(out_path, self._real_work_dir)
        except Exception:
            rel_out = str(out_path)
        summary_parts = [f"rendered={result.ok}", f"output={rel_out}"]
        if result.error:
            summary_parts.append(f"error={result.error}")
        if result.extracted:
            summary_parts.append(f"extracted_keys={list(result.extracted.keys())[:10]}")
