...
class LoadImage(Action):
    action_type: str = field(default="load_image", init=False, repr=False)
    ...
    def get_action_description(cls) -> str:
        return """
## LoadImage
Signature: LoadImage(file_path="path/to/image.png")
Description: Read an image from disk and return it as a base64 data URI so the model can view it. Supports PNG/JPG.
"""
    

@dataclass
class RenderChart(Action):
    action_type: str = field(default="render_chart", init=False, repr=False)
    file_path: str = field(default="")
    tool_type: str = field(default="")
    output_path: str = field(default="result.png")

    def __repr__(self) -> str:
        return (
            f'{self.__class__.__name__}(file_path="{self.file_path}", '
            f'tool_type="{self.tool_type}", output_path="{self.output_path}")'
        )
    ...
    def get_action_description(cls) -> str:
        return """
## RenderChart
Signature: RenderChart(file_path="result.json", tool_type="echarts", output_path="result.png")
Description: Render a visualization spec/snippet (ECharts, Vega-Lite, D3.js, Plotly.js) to a PNG image using Playwright. The rendered image is saved to `output_path` (default result.png).
"""
...
"""
        return """
## Terminate Action
* Signature: Terminate(report_path="report.md", content="可选信息")
* Description: 终止任务，不需要写报告。
"""
